...
# STEMMING ALGORITHMS & PREPROCESSING
import re
...
def preprocess2(text_string):
	...
	stemmed_words = [stemmer2.stem(word) for word in parsed_text.split()]
...
print ('English stop words Mean Accuracy: %.3f (%.3f)' % (mean(scores1), std(scores1)))
...
def generateNGramValues2(X_train):
	count_vect = CountVectorizer(ngram_range=(1, 3), stop_words=final_stopwords_list, # STOP WORDS ARE ADDED HERE
		preprocessor=preprocess, lowercase=True) # CHANGE TO PREPROCESS 2 TO SEE THE EFFECT OF STEMMING ALGORITHMS
	X_train_counts = count_vect.fit_transform(X_train)
	return [X_train_counts, count_vect]


df_train2 = pd.read_csv("../data/datasets/en-fr/en_fr_train.csv", index_col=False,
    lineterminator='\n')
X_train2, Y_train2 = getdata(df_train2)
X_train2 = df_train2['tweet']
Y_train2 = df_train2['class']

X_train_counts2, count_vect = generateNGramValues2(X_train2)
...
print('No stop words Mean Accuracy: %.3f (%.3f)' % (mean(scores2), std(scores2)))
print(scores1)
print(scores2)
